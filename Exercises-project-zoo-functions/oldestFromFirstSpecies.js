...
// function firstSpecieForEmployee(id) {
//     return employees.find(select => select.id === id).responsibleFor[0];
// } 
// console.log(firstSpecieForEmployee('c5b83cb3-a451-49e2-ac45-ff3f54fbe7e1'));

//param da função é o id de funcionário
function oldestFromFirstSpecies(id) {
    //função que traz o primeiro id da espécie
    ...
    //acima, ele abre a variável de animais, e pesquisa se tem alguma espécie com id igual ao retorno da função, e pega os residentes.
    const newList = [...specie].sort((a, b) => a.age - b.age); //ordena os animais residentes pelo mais velho entre eles.
    console.log(newList) //aqui percebi que tinha que pegar os dados do último objeto.
    const { name, sex, age } = newList[newList.length - 1]; //tive que fazer isso para pegar os dados do animal mais velho. obs: porque não consegui ordenar pelo método sort!
    const list = [name, sex, age];
    return list;
}
console.log(oldestFromFirstSpecies('c5b83cb3-a451-49e2-ac45-ff3f54fbe7e1'))

// const newList = [...specie].sort((a, b) => a.age < b.age); //traria o animal mais velho 
// const { name, sex, age } = newList[0]; traria o animal mais velho aqqui. 