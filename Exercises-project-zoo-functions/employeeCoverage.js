...
// parte 1 Sem parâmetros, retorna uma lista de funcionários e os animais pelos quais eles são responsáveis
function employeeCoverage() {
    let result = {};
    employees.forEach((employee) => {
        result[`${employee.firstName} ${employee.lastName}`] = employee.responsibleFor.map((ids) => {
            return animals.find(searchId => searchId.id === ids).name;
        })
    })
    return result;
}
...
// parte 2 Com o id de um funcionário, retorna os animais pelos quais o funcionário é responsável
function employeeCoverage1(id) {
    let result = {};
    employees.forEach((employee) => {
        if (employee.id === id) {
            result[`${employee.firstName} ${employee.lastName}`] = employee.responsibleFor.map((ids) => {
...
// console.log(employeeCoverage1('c5b83cb3-a451-49e2-ac45-ff3f54fbe7e1'))

// parte 3 Com o primeiro nome de um funcionário, retorna os animais pelos quais o funcionário é responsável
// parte 4 Com o último nome de um funcionário, retorna os animais pelos quais o funcionário é responsável
function employeeCoverage2(idOrName) {
    let result = {};
    if (!idOrName) return employeeCoverage(); //se !idOrName
    employees.forEach((employee) => {
...
console.log(employeeCoverage2('Orloff'));